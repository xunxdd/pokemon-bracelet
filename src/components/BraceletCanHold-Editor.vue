    ...
    submit() {
      this.$validator.validate().then(result => {
        ...
        if (this.key) {
          ...
          return this.goToPokeStatCard(this.key)
        }
        this.stat_props.time_added = Date.now()
        PokeService.save(this.stat_props)
          .then((newstat) => {
            this.goToPokeStatCard(newstat.key)
          })
          .catch(error => {
            console.log(error)
          })
          ...
          <v-flex xs12>
            <v-card class="grey--text">
              <form>
                ...
                <v-text-field v-model="stat_props.evolves_into" label="Evolves Into" data-vv-name="evolves_into"></v-text-field>
